            ...
            if (data.data.length != 0) {
                for (var i = 0; i < data.data.length; i++) {
                    function put(i) {
                        ...
                        setTimeout(function () {
                            $('#' + data.data[i].id).addClass("list_show");
                        }, i * 500);
                    }
                    put(i);
                }
                if (data.paging.totals == data.paging.current_page) {
                    ...
                    $('#more').html('<a href="javascript:void(0);" onclick="get_list_btn()" id="next">点击加载更多...</a>')
                }
            } else {
                $('#more').html("文章加载失败了...")
            }
            setTimeout(function () {
                $('#more').addClass("list_show");
            }, (data.data.length + 1) * 400);
        }
    });
}
